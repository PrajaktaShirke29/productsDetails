...
class ProductEditPage extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.match.params.mode === 'edit') {
      axios
        .get('http://localhost:3100/products/' + this.props.match.params.id , { headers: { Authorization: 'Bearer '.concat(localStorage.getItem('token')) } })
  ...
  editProductHandler = event => {
    ...
    if (this.props.match.params.mode === 'edit') {
      ...
      request = axios.post('http://localhost:3100/products', productData, { headers: headers});
    }
    request
      .then(result => {
        this.setState({ isLoading: false });
